...
namespace TSL.Collisions {
  ...
  public interface ICollisionCaster {
    ...
    /// <seealso cref="CollisionCaster.IsTopByRightWall"/>
  ...
  public class CollisionCaster : ICollisionCaster {
    ...
    #region Constructors
    public CollisionCaster() {
      layerMask = LayerMask.GetMask("Foreground");
    }

    public CollisionCaster(Collider2D parent) : this() {
      parentCollider = parent;
    }

    public CollisionCaster(float colliderWidth, float boxCastMargin, string[] layerNames) {
      this.colliderWidth = colliderWidth;
      this.boxCastMargin = boxCastMargin;
      layerMask = LayerMask.GetMask(layerNames);
    }

    public CollisionCaster(string[] layerNames) {
      layerMask = LayerMask.GetMask(layerNames);
    }
